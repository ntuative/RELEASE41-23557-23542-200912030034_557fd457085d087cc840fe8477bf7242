package deng.fzip
{
   ...
   public class FZip extends EventDispatcher
   {
         ...
         if(param1.bytesAvailable >= 4)
         {
            ...
            switch(_loc2_)
            {
               case 67324752:
                  ...
                  var_460 = new FZipFile(charEncoding);
                  break;
               case 33639248:
               case 101010256:
                  parseFunc = parseIdle;
      ...
      protected function progressHandler(param1:Event) : void
      {
         ...
         {
            close();
            if(!hasEventListener(FZipErrorEvent.const_513))
            {
               throw e;
            }
            dispatchEvent(new FZipErrorEvent(FZipErrorEvent.const_513,e.message));
         }
      }
      ...
      public function loadBytes(param1:ByteArray) : void
      {
         if(!var_97 && parseFunc == parseIdle)
         {
            ...
            {
               dispatchEvent(new FZipErrorEvent(FZipErrorEvent.const_513,"EOF"));
            }
         }
      }
      ...
      {
         if(var_460.parse(param1))
         {
            var_53.push(var_460);
            if(false)
            {
               var_135["null"] = var_460;
            }
            dispatchEvent(new FZipEvent(FZipEvent.const_1167,var_460));
            var_460 = null;
