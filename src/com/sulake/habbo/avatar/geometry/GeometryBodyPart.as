package com.sulake.habbo.avatar.geometry
{
   ...
   public class GeometryBodyPart extends Node3D
   {
      ...
      public function GeometryBodyPart(param1:XML)
      {
         ...
         var_1205 = parseFloat(param1.@radius);
         _id = String(param1.@id);
         var_522 = new Dictionary();
         for each(_loc2_ in param1..item)
         {
            _loc3_ = new GeometryItem(_loc2_);
            var_522[String(_loc2_.@id)] = _loc3_;
         }
      }
      ...
      {
         var _loc1_:* = null;
         for each(_loc1_ in var_522)
         {
            if(_loc1_ != null)
            {
               if(_loc1_.isDynamic)
               {
                  var_522[_loc1_.id] = null;
                  _loc1_ = null;
               }
            }
         }
      ...
      {
         var _loc2_:Number = Math.abs(param1.z - this.transformedLocation.z - var_1205);
         var _loc3_:Number = Math.abs(param1.z - this.transformedLocation.z + var_1205);
         return Math.min(_loc2_,_loc3_);
      }
      
      public function hasPart(param1:String) : Boolean
      {
         var _loc2_:GeometryItem = var_522[param1];
         return _loc2_ != null;
      }
