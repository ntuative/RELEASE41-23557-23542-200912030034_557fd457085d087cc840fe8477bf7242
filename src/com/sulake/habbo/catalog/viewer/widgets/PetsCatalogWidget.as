package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget
   {
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(var_382 >= var_288.length)
         {
            return "";
         }
         var petColor:IPetColor = var_288[var_382] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = var_533[var_1412] as IBreed;
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         var_382 = param1.index;
         if(var_382 < 0 || var_382 > var_288.length)
         {
            var_382 = 0;
         }
         updateImage();
      ...
      private function onApproveNameResult(param1:CatalogWidgetApproveNameResultEvent) : void
      {
         ...
         (page.viewer.catalog as HabboCatalog).showPurchaseConfirmation(var_927.getWithIndex(0),page,p);
      }
      ...
      override public function init() : void
      {
         ...
         _loc2_.addEventListener(WindowEvent.const_228,onTextWindowEvent);
         var_927 = new Map();
         ...
         var_763 = getPetTypeIndexFromProduct(_loc5_.localizationId);
         var_288 = [];
         var_533 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc11_ = _loc10_.avatarRenderManager.petDataManager.getPetData(var_763);
               if(_loc11_)
               {
                  var_533 = _loc11_.breeds;
                  var_288 = _loc11_.colors;
                  if(var_288 == null)
                  {
                     var_288 = [];
                  }
         ...
         var _loc6_:int = 0;
         while(_loc6_ < var_533.length)
         {
            _loc12_ = var_533[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(getRaceLocalizationKey(var_763,_loc12_.id),getRaceLocalizationKey(var_763,_loc12_.id)));
            _loc6_++;
         }
         var_927.add(var_763,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_862,onApproveNameResult);
         _loc5_ = var_927.getWithIndex(0);
         if(_loc5_ != null)
         {
            events.dispatchEvent(new SelectProductEvent(_loc5_));
         }
         var _loc8_:* = [];
         for each(_loc9_ in var_288)
         {
      ...
      {
         var _loc7_:* = null;
         var _loc1_:Offer = var_927.getWithIndex(0);
         if(_loc1_ == null)
         {
            return;
         }
         ...
         if(var_382 >= 0 && var_382 < var_288.length)
         {
            _loc7_ = var_288[var_382] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = var_533[var_1412] as IBreed;
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImageNew(var_763,_loc3_.id,_loc2_,AvatarScaleType.const_50);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_274,AvatarAction.const_580);
