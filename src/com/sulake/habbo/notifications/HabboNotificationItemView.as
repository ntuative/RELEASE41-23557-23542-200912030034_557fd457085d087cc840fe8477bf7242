package com.sulake.habbo.notifications
{
   ...
   public class HabboNotificationItemView implements ICoreUpdateReceiver
   {
      ...
      public static const const_683:int = 70;
      ...
      private static const const_347:int = 1;
      ...
      public function HabboNotificationItemView(param1:IAsset, param2:IHabboWindowManager, param3:Map, param4:Map, param5:IHabboToolbar)
      {
         ...
         if(_loc7_ != null)
         {
            var_1167 = 0 - _loc7_.rectangle.bottom;
         }
         else
         {
            var_1167 = 15;
      ...
      private function setNotificationText(param1:String) : void
      {
         ...
         _window.height = 0;
         _loc2_.text = param1;
         _loc2_.height = _loc2_.textHeight + var_1167;
         if(_window.height < var_1422)
         {
            _window.height = var_1422;
         }
      }
      ...
      public function update(param1:uint) : void
      {
         ...
         switch(_state)
         {
            ...
            case const_347:
               var_770 += param1;
               _loc2_ = Number(var_770) / Number(var_384["time_fade_in"]);
               if(var_770 > int(var_384["time_fade_in"]))
               {
                  startDisplay();
               }
               adjustBlend(_loc2_);
            ...
            case const_746:
               var_952 += param1;
               if(var_952 > int(var_384["time_display"]) && !var_1168)
               {
                  startFadeOut();
               }
               break;
            case const_747:
               var_771 += param1;
               _loc3_ = 1 - Number(var_771) / Number(var_384["time_fade_out"]);
               adjustBlend(_loc3_);
               if(var_771 > int(var_384["time_fade_in"]))
               {
                  startIdling();
               }
         }
      ...
      public function reposition(param1:int = -1) : void
      {
         ...
         var _loc3_:int = var_198.orientation == "TOP" ? int(const_338) : 0;
         var _loc4_:int = var_198.orientation == "BOTTOM" ? int(_loc2_.height - const_338) : int(_loc2_.height);
         var _loc5_:int = var_198.orientation == "LEFT" ? int(const_338) : 0;
         var _loc6_:int = var_198.orientation == "RIGHT" ? int(_loc2_.width - const_338) : int(_loc2_.width);
         if(false)
         {
            if(false)
            {
               _window.x = _loc5_ + const_379;
            }
            else
            {
               _window.x = _loc6_ - 0 - const_379;
            }
         }
         else if(false)
         {
         ...
         if(false)
         {
            if(false)
            {
               _window.y = _loc3_ + var_535;
            }
            else
            {
               _window.y = _loc4_ - 0 - var_535;
            }
         }
         else if(false)
         {
