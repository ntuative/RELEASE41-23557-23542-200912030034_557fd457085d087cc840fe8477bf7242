package com.sulake.habbo.friendlist.domain
{
   ...
   public class FriendCategories
   {
      ...
      private var var_403:IFriendCategoriesDeps;
      ...
      private function updateCategories(param1:Array) : void
      {
         ...
         var _loc5_:* = null;
         flushReceivedStatus();
         findCategory(FriendCategory.const_420).received = true;
         findCategory(FriendCategory.const_947).received = true;
      ...
      public function onFriendListUpdate(param1:IMessageEvent) : void
      {
         ...
         for each(_loc4_ in _loc2_.updatedFriends)
         {
            ...
            var_403.messenger.setFollowingAllowed(_loc4_.id,_loc4_.followingAllowed && _loc4_.online);
            _loc6_ = isFriendOnline(_loc4_.id);
            if(_loc6_ && !_loc4_.online)
            {
               var_403.messenger.setOnlineStatus(_loc4_.id,_loc4_.online);
               var_403.notifications.addOfflineNotification(_loc4_.name,_loc4_.realName);
            }
            if(!_loc6_ && _loc4_.online)
            {
               var_403.messenger.setOnlineStatus(_loc4_.id,_loc4_.online);
               var_403.view.setNewMessageArrived();
               var_403.notifications.addOnlineNotification(_loc4_.name,_loc4_.figure,_loc4_.realName);
            }
      ...
      {
         return var_887[param1];
      }
      
      private function flushReceivedStatus() : void
      {
         var _loc1_:* = null;
         for each(_loc1_ in var_337)
         {
      ...
      {
         var _loc2_:int = !!param1.online ? int(param1.categoryId) : int(FriendCategory.const_420);
         var _loc3_:FriendCategory = findCategory(_loc2_);
         if(_loc3_ != null)
         {
            _loc3_.addFriend(param1);
            var_887[param1.id] = param1;
            return _loc3_;
         }
